...
def main():
    ...
    parser.add_argument('-n','--n_mono1', help='atom number in monomer 1', type=int)
    ...
    parser.add_argument(
        '-ah', '--apta', help='perform atomic pair transfer analysis of HOMO', action='store_true'
    )
    parser.add_argument(
        '-al', '--apta_l', help='perform atomic pair transfer analysis of LUMO', action='store_true'
    )
    parser.add_argument(
        '-anh', '--apta_nh', help='perform atomic pair transfer analysis of next HUMO', action='store_true'
    )
    parser.add_argument(
        '-ahl', '--apta_hl', help='perform atomic pair transfer analysis of HUMO and LUMO', action='store_true'
    )
    parser.add_argument(
    ...
    tcal = Tcal(args.file,args.n_mono1)
    ...
    try:
        ...
        if args.apta_nh:
            analyze_orbital = 'NHOMO'
        if args.apta_hl:
            analyze_orbital = 'HOMO-LUMO'
        
        if args.apta or args.apta_l or args.apta_nh or args.apta_hl:
            if args.output:
                output_apta = True
            else:
                output_apta = False
...
class Tcal:
    ...
    def __init__(self, file,n_mono1):
        """Inits Tcalculass.
    ...
    def atomic_pair_transfer_analysis(self, analyze_orbital='HOMO', output_apta=False):
        ...
        elif analyze_orbital.upper() == 'NHOMO':
            orb1 = self.mo1[self.n_elect1 - 2]
            orb2 = self.mo2[self.n_elect2 - 2]
        elif analyze_orbital.upper() == 'HOMO-LUMO':
    ...
    def convert_xyz_to_gjf(self, function='B3LYP/6-31G(d,p)', nprocshared=40, mem=15, unit='GB'):
    ...
    def create_monomer_file(self):
        ...
        mono_coordinates = coordinates[:self.n_mono1]
    ...
    def print_apta(self, a_transfer):
        ...
        apta = []
        tmp_list = ['atom']
        for i in range(self.n_mono1, n_atoms):
            tmp_list.append(f'{i+1}{labels[i]}')
        tmp_list.append('sum')
        apta.append(tmp_list)

        for i in range(self.n_mono1):
            tmp_list = []
            tmp_list.append(f'{i+1}{labels[i]}')
            for j in range(self.n_mono1, n_atoms):
    ...
    def print_transfer_integrals(self):
        ...
        transfer = Tcal.cal_transfer_integrals(
            self.mo1[self.n_elect1 - 1], self.overlap, self.fock, self.mo2[self.n_elect2]
        )
        print(f'HOMO-LUMO\t{transfer:.3f}\tmeV')
        
        transfer = Tcal.cal_transfer_integrals(
            self.mo1[self.n_elect1 - 1], self.overlap, self.fock, self.mo2[self.n_elect2 - 2]
        )
        print(f'HOMO-NHOMO\t{transfer:.3f}\tmeV')
        
        transfer = Tcal.cal_transfer_integrals(
            self.mo1[self.n_elect1 - 2], self.overlap, self.fock, self.mo2[self.n_elect2 - 1]
        )
        print(f'NHOMO-HOMO\t{transfer:.3f}\tmeV')
    ...
    def read_dimer(self, is_matrix=False, output_matrix=False):
        ...
        zeros_matrix1 = np.zeros((self.n_basis1, self.n_basis2))
        zeros_matrix2 = np.zeros((self.n_basis2, self.n_basis1))
        self.mo1 = np.hstack([self.mo1.T, zeros_matrix1])
        self.mo2 = np.hstack([zeros_matrix2, self.mo2.T])
...
class PairAnalysis:
    ...
    def __init__(self, apta):
        ...
        self._mono1_atom_num = len(self._labels)
        label = apta[0][1:-1]
        label = list(map(lambda x: re.sub('[0-9]+', '', x), label))
        self._mono2_atom_num = len(label)
    ...
    def print_largest_pairs(self):
        ...
        for i, a_i in enumerate(reversed(print_index)):
            row_i = a_i // self._mono2_atom_num
            col_i = a_i % self._mono2_atom_num
            pair = f'{row_i+1}{self._labels[row_i]}' + ' - ' + \
                f'{col_i+self._mono1_atom_num+1}{self._labels[col_i+self._mono1_atom_num]}'
    ...
    def print_element_pairs(self):
        """Print element pairs."""
        element_pair = {
            'H-I': 0.0, 'C-C': 0.0, 'C-H': 0.0, 'C-S': 0.0, 'C-O': 0.0, 'C-Se': 0.0,
            'C-Cl': 0.0, 'C-I': 0.0, 'S-S': 0.0, 'S-O': 0.0, 'S-Cl': 0.0, 'Se-Se': 0.0, 'N-S': 0.0, 'I-S': 0.0,
            'I-I': 0.0,
        }
        keys = element_pair.keys()

        for i in range(self._mono1_atom_num):
            sym1 = self._labels[i]
            for j in range(self._mono2_atom_num):
                sym2 = self._labels[self._mono1_atom_num + j]
